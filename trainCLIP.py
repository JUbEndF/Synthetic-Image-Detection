...
from networks.modelCLIP.AIDetectionCLIPModel import( 
    ClassifierResNet, Classifierclip_deit_base_patch16_224_L7
    )
...
if __name__ == '__main__':
    ...
    if True:
        train_loader = loadDataCLIPFILE("clip_vit_16_features_train.npz")
        val_loader   = loadDataCLIPFILE("clip_vit_16_features_val.npz")
    else:
        train_loader = load_data(args.train_data_path)
        val_loader = load_data(args.val_data_path)

    for models in [Classifierclip_deit_base_patch16_224_L7, ClassifierResNet]:
        model = models().to(args.device)
