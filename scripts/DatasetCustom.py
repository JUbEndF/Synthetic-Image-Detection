...
def LoadDataSet(folder_path, device):
    ...
    for image_path in tqdm(all_image_paths):
        features.append(dftImage(image_path, device))
...
class CustomDataset(Dataset):
    def __init__(self, data_folder_path, device):
        self.X, self.y = LoadDataSet(data_folder_path, device)
        #self.device = device
    def __len__(self):
        return len(self.X)
    ...
    def __getitem__(self, idx):
        ...
        image_features_tensor = torch.tensor(imageDFT)
